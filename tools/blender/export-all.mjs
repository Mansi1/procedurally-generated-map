// Exportiert die Clips aller Blender-Dateien in assets/blender/clips/ nach
...
for (const file of readdirSync(`${root}assets/blender/clips`).filter((f) => f.endsWith('.blend'))) {
  const name = file.replace(/\.blend$/, '');
  const out = `${root}src/models/${name}_clips`;
  const log = execFileSync(blender, [
    '-b', `${root}assets/blender/clips/${file}`,
