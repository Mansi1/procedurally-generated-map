...
export class TileProbe {
  ...
  private generateResources(tile: MapTile): { type: ResourceType; amount: number } {
    ...
    if (tile.tileType === "mountain" && r > 0.25) {
      return { type: r > 0.86 ? "gold" : "stone", amount: Math.floor((r + 1) * 40) };
    }
...
/** Hauptansicht. Das Gelände entsteht komplett im Fragment-Shader. */
export class MapRenderer {
  ...
  render(cameraX: number, cameraY: number, mouseTileX?: number, mouseTileY?: number) {
    // cameraX/Y sind CSS-Pixel; ein Geräte-Pixel deckt so viele Welt-Tiles ab:
    const tilesPerPixel = 1 / (this.tileSize * this.pixelRatio);
    ...
    this.terrain.render(cameraX / this.tileSize, cameraY / this.tileSize, tilesPerPixel);
  }
}
...
export class MiniMap {
  ...
  /** So viel Welt zeigt sie mindestens, damit sie beim Hineinzoomen nützlich bleibt. */
  ...
  /** Welt-Tiles, die die Minimap gerade abdeckt. */
  coverage(viewportTilesX: number): number {
    return Math.max(viewportTilesX * MiniMap.OVERVIEW, MiniMap.MIN_COVERAGE);
  }

  render(camTopLeftTileX: number, camTopLeftTileY: number, viewportTilesX: number, viewportTilesY: number) {
    const cover = this.coverage(viewportTilesX);
    ...
    this.terrain.viewRect = {
      x: camTopLeftTileX,
      y: camTopLeftTileY,
      width: viewportTilesX,
      height: viewportTilesY,
    };
    this.terrain.render(centerX - cover / 2, centerY - cover / 2, cover / this.canvas.width);
  }
  ...
  toWorld(clickX: number, clickY: number, camTopLeftTileX: number, camTopLeftTileY: number,
    ...
    const centerX = camTopLeftTileX + viewportTilesX / 2;
    const centerY = camTopLeftTileY + viewportTilesY / 2;
    const perCssPixel = cover / this.cssSize;
    return {
      x: centerX - cover / 2 + clickX * perCssPixel,
      y: centerY - cover / 2 + clickY * perCssPixel,
    };
  }
}
